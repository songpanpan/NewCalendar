...
public class NongLi {
    ...
    private static String formatYear(int year) {
        ...
        for(int year2 = year; year2 != 0; year2 /= 10) {
            int every = year2 % 10;
    ...
    private static String formatMonth(int month) {
        ...
        return month > 12 ? "闰" + table.substring(month - 13, month - 12) : table.substring(month - 1, month);
    }
    ...
    private static int[] cast(String start, String now, int[] bigOrLitter, int leap) {
        ...
        int i;
        for(i = 0; dif >= sum; sum += bigOrLitterSort[i++] + 29) {
        }

        int year = Integer.parseInt(now.substring(0, 4));
        int[] result = new int[]{dif < 0 ? year - 1 : year, i - 2 <= 0 ? i + 10 : i - 2, 0};
        if (dif >= 0) {
            if (leap != 0) {
                if (result[1] == leap + 1) {
                    result[1] += 11;
            ...
            String data = dataInit[year - startYear];
            ...
            if (lastLeap != 0) {
                ...
                } else if (lastLeap == 11 && result[1] == 12 || lastLeap == 12 && result[1] == 11) {
                    result[1] = 12;
                }
            }
        }
    ...
    private static int[] cast(String now, String data) {
        ...
        int leap;
        for(leap = 0; leap < bigOrLitter.length; ++leap) {
            bigOrLitter[leap] = Integer.parseInt(bigOrLitterStr.substring(leap, leap + 1));
        }

        leap = Integer.parseInt(leapStr);
    ...
    private static int[] judge(String date) {
        ...
        if (date.matches("\\d{8}")) {
            ...
            if (date.matches("\\d+-\\d{1,2}-\\d{1,2}")) {
                ...
                if (!date.matches("\\d+年\\d{1,2}月\\d{1,2}日")) {
                    return null;
                }

                dateArray = date.split("年|月|日");
                year = Integer.parseInt(dateArray[0]);
                month = Integer.parseInt(dateArray[1]);
                day = Integer.parseInt(dateArray[2]);
            }
    ...
    private static String[] read(String fileName) {
        ...
        try {
            br = new BufferedReader(new InputStreamReader(com.necer.ncalendar.tools.NongLi.class.getClassLoader().getResourceAsStream("assets/config.cal")));
            String every = null;
